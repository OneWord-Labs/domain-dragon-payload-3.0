...
export function serializeLexical({ nodes }: Props): ReactNode {
  return (
    <Fragment>
      {nodes?.map((_node, index): ReactNode | null => {
        if (_node.type === 'text') {
        ...
        const serializedChildrenFn = (node: SerializedElementNode): ReactNode | null => {
        ...
        if (_node.type === 'block') {
          ...
          switch (_node.type) {
            ...
            case 'heading': {
              ...
              type Heading = Extract<keyof any, 'h1' | 'h2' | 'h3' | 'h4' | 'h5'>
            ...
            case 'list': {
              ...
              type List = Extract<keyof any, 'ol' | 'ul'>
              const Tag = node?.tag as any
              return (
